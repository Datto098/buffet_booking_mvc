<?php
/**
 * Script to add sample news data to the news table
 */
...
try {
    ...
    echo "<h1>Adding Sample News Data</h1>";
    ...
    if ($result['count'] > 0) {
        echo "<p>✅ News data already exists ({$result['count']} articles found)</p>";
    } else {
        echo "<h2>Adding sample news articles...</h2>";

        // Get or create admin user ID for author
...
<p>Make your reservations early as space is limited for our special holiday events. We look forward to celebrating with you!</p>',
    ...
    echo "<h2>Sample news data setup completed successfully!</h2>";
