...
<body class="admin-page news-index">
    ...
    function toggleStatus(newsId, newStatus) {
        ...
        fetch('<?= SITE_URL ?>/admin/news/toggle-status', {
            method: 'POST',
            headers: {
    ...
    function deleteNews(newsId) {
        ...
        fetch('<?= SITE_URL ?>/admin/news/delete', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'X-CSRF-Token': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
    ...
    function executeBulkAction() {
        ...
        const selectedIds = Array.from(document.querySelectorAll('.news-checkbox:checked'))
                                .map(cb => cb.value);
        ...
        if (!confirm(`Are you sure you want to ${action} ${selectedIds.length} article(s)?`)) {
            return;
        }
    ...
    function exportNews() {
        ...
        window.location.href = `<?= SITE_URL ?>/admin/news?${params.toString()}`;
    }
