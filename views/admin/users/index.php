...
<body>
                                    ...
                                    <tbody>
                                        <?php foreach ($users as $user): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    <strong class="text-primary">#<?= htmlspecialchars($user['id']) ?></strong>
                                                </td>
                                                <td>
                                                    <div class="d-flex align-items-center">
                                                        ...
                                                        <div>
                                                            <div class="fw-medium"><?= htmlspecialchars($user['full_name'] ?? $user['username']) ?></div>
                                                ...
                                                <td>
                                                    ...
                                                    $roleIcon = $user['role'] == 'admin' ? 'user-shield' : 'user';
                                                    ?>
                                                ...
                                                <td>
                                                    ...
                                                    $statusIcon = $user['status'] == 'active' ? 'check-circle' : 'times-circle';
                                                    ?>
                                ...
                                <nav aria-label="Users pagination">
                                    <ul class="pagination">
                                        <?php if (($pagination['current_page'] ?? 1) > 1): ?>
                                            <li class="page-item">
                                                <a class="page-link" href="?page=<?= ($pagination['current_page'] ?? 1) - 1 ?>">
    ...
    <script>
        ...
        function bulkUpdateStatus(status) {
            ...
            if (confirm(`Are you sure you want to ${status} ${selectedUsers.length} user(s)?`)) {
                const form = document.createElement('form');
                form.method = 'POST';
                form.action = '<?= SITE_URL ?>/admin/users/bulk-update-status';
