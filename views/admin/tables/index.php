                        ...
                        <?php foreach ($data['locationStats'] as $location): ?>
                            <div class="col-md-2 col-sm-6 mb-3">
                                <div class="text-center p-3 border rounded">
                                    ...
                                    <?php
                                    $fullLocation = $location['location'] ?: 'Main Area';
                                    $shortLocation = mb_strlen($fullLocation) > 10 ? mb_substr($fullLocation, 0, 10) . '...' : $fullLocation;
                                    ?>
                                    <h6 class="mb-1" title="<?= htmlspecialchars($fullLocation) ?>">
...
<div class="card shadow">
    ...
    $normalizedLocation = normalize($table['location']);
    $isLocked = isset($addressStatus[$normalizedLocation]) && ((string)$addressStatus[$normalizedLocation] === '0');
?>
                            <tr>
                                <td>
                                ...
                                <td>
                                    <span class="badge bg-secondary" title="<?php echo htmlspecialchars($table['location']); ?>">
                                        <i class="fas fa-map-marker-alt"></i>
                                        <?php
                                        $shortLocation = mb_strlen($table['location']) > 20 ? mb_substr($table['location'], 0, 20) . '...' : $table['location'];
                                        echo htmlspecialchars($shortLocation);
                                        ?>
                                    </span>
                                </td>
                                <td>
    ...
    <?php
    $normalizedLocation = normalize($table['location']);
    $isAddressLocked = isset($addressStatus[$normalizedLocation]) && ((string)$addressStatus[$normalizedLocation] === '0');
    ?>
    <div class="form-check form-switch">
        <input class="form-check-input" type="checkbox"
               id="status_<?php echo $table['id']; ?>"
               <?php echo ($isAddressLocked || !$table['is_available']) ? '' : 'checked'; ?>
               <?php echo $isAddressLocked ? 'disabled' : ''; ?>
               onchange="toggleTableStatus(<?php echo $table['id']; ?>, this.checked)">
        <label class="form-check-label" for="status_<?php echo $table['id']; ?>">
            <span class="badge bg-<?php echo ($isAddressLocked || !$table['is_available']) ? 'secondary' : 'success'; ?>">
                <?php echo ($isAddressLocked || !$table['is_available']) ? 'Unavailable' : 'Available'; ?>
            </span>
        </label>
    </div>
</td>
...
if (!empty($data['addresses'])) {
    foreach ($data['addresses'] as $addr) {
        $normalizedAddr = normalize($addr['address']);
        $addressStatus[$normalizedAddr] = $addr['status'];
    }
}
?>
<script>
